...
3. Soạn Revision mới (`draft --chapter N` / `branch --name ...`) + Trích xuất Facts
   (Áp dụng nghiêm ngặt: vietnamese_novel_prose.md & anti_ai_tone.md)
         │
         ▼
...
```
* **Bắt buộc tuân thủ:**
  - Quy chuẩn văn phong thuần Việt: [`vietnamese_novel_prose.md`](./vietnamese_novel_prose.md).
  - Quy chuẩn chống văn mẫu AI & văn convert: [`anti_ai_tone.md`](./anti_ai_tone.md).
...
* Tạo báo cáo so sánh giữa bản cũ và bản mới:
  ...
  ```
* Chấm điểm 7 chiều theo [`quality_checklist.md`](./quality_checklist.md):
  ```bash
